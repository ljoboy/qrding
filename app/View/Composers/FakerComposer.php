...
final class FakerComposer
{
    public function fakeUsers()
    {
        ...
        return $users->random(3)->map(function ($user) {
            return [
                'name' => $user['name'],
                'gender' => $user['gender'],
                'email' => mb_strtolower(str_replace(' ', '', $user['name']) . '@left4code.com'),
            ];
        });
    }
    ...
    public function fakeDates()
    {
        $dates = [];
        for ($i = 0; $i < 5; $i++) {
            $dates[] = date('j F Y', (int) (mt_rand(1586584776897, 1672333200000) / 1000));
        }
    ...
    public function fakeCategories()
    {
        ...
        return $categories->shuffle()->map(function ($category) {
            return [
                'name' => $category['name'],
                'tags' => $category['tags'],
                'slug' => str_replace('&', 'and', str_replace(' ', '-', mb_strtolower($category['name']))),
            ];
        });
    }
